...
public class TransactionProcessingEngine {
    ...
    public TransactionProcessingEngine(String theReferenceId, String theSourceMsisdn, String theRequestKeyword, String theRequestCommand) {
        ...
        try {
            ...
            hostIpAddress = (hostInetAddress != null) ? hostInetAddress.getHostAddress() : "127.0.0.1";
        }
    ...
    public void processRequestCommand() {
        try {
            ...
            /*
             * switch (requestKeyword.toUpperCase()) { case "KYCR": processKYCR(); break; case "DPST": processDPST();
             * break; case "MBAL": processMBAL(); break; case "CASH": processCASH(); break; case "WDRW": processWDRW();
             * break; case "STMT": processSTMT(); break; case "PINC": processPINC(); break; case "ACTV": processACTV();
             * break; case "CRGL": processCRGL(); break; case "MTOM": processMTOM(); break; case "MTOU": // Use
             * processMTOM() for both MTOM and MTOU processMTOM(); break; case "GBAL": processGBAL(); break; case
             * "TENQ": processTENQ(); break; case "REDM": processREDM(); break; case "STOP": processSTOP(); break; case
             * "REBT": processREBT(); break; default: destMessage =
             * "The Service failed to determine the kind of transaction requested.";
             * HelperUtils.writeToLogFile("console", destMessage); HelperUtils.writeToLogFile("OutBoundMessages",
             * destMessage); // send the SMS HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId); break; }
             */
            ...
            else if (requestKeyword.toUpperCase().equalsIgnoreCase("NTINVITE")) {
                processNTINVITE();
            }
            else if (requestKeyword.toUpperCase().equalsIgnoreCase("NTRESPINV")) {
                processNTRESPINV();
            }
            else {
     ...
     */
    private static boolean logInBoundMessage(String sourceMsisdn, String referenceId, String requestKeyword, String requestCommand)
            throws SQLException {
        Connection cn = null;
        ...
        try {
            ...
            if (sourceMsisdn.trim().isEmpty() || referenceId.trim().isEmpty() || requestCommand.trim().isEmpty()) {
                return false;
            }
            ...
            stm = cn.prepareStatement(String.format(sqlQuery, referenceId, sourceMsisdn, requestKeyword, requestCommand));
    ...
    private boolean logInBoundMessage(String theMessage) {
        try
        {
            ...
            while (logInBoundMessage(sourceMsisdn, referenceId, requestKeyword, theMessage) == false) {
                if (insertAttempts >= 5) {
                    HelperUtils.writeToLogFile("Server", "Failed Completely in all Attempts to Insert into SMPP_HITS.");
                    return false;
                }
        ...
        try {
            ...
            if (theMobileNo.startsWith(SystemConfigInfo.getCountryCode())) {
                if (SystemConfigInfo.getMsisdnLeadZeroRequired()) {
                    ...
                    if (theMobileNo.trim().length() == (SystemConfigInfo.getMobileLength() + SystemConfigInfo.getCountryCode().length() + 1)) {
                        theMsisdn = theMobileNo;
                    }
                }
                else {
                    if (theMobileNo.trim().length() == (SystemConfigInfo.getMobileLength() + SystemConfigInfo.getCountryCode().length())) {
                        theMsisdn = theMobileNo;
                    }
                }
            }
                ...
                if (SystemConfigInfo.getMsisdnLeadZeroRequired()) {
                    ...
                    if (theMobileNo.trim().length() == (SystemConfigInfo.getMobileLength() + SystemConfigInfo.getCountryCode().length() + 1)) {
                        theMsisdn = SystemConfigInfo.getCountryCode().concat(theMobileNo);
                    }
                }
                else {
                    if (theMobileNo.startsWith("0")) {
                    ...
                    if (theMobileNo.trim().length() == SystemConfigInfo.getMobileLength()) {
                        theMsisdn = SystemConfigInfo.getCountryCode().concat(theMobileNo);
                    }
                }
            }
    ...
    private boolean validatePinCode(String thePinCode) {
        ...
        try {
            ...
            if (this.sourceCustInfo.getPinCode() == null || !this.sourceCustInfo.getPinCode().equalsIgnoreCase(thePinCode)) {
                failedPinAttempts++;

                if (failedPinAttempts >= SystemConfigInfo.getInvalidPasswordLock()) {
                    CustomerInformation.lockCustomer(this.sourceCustInfo.getCustomerId());
                }
                isValid = false;
            }
            else {
                isValid = true;
                CustomerInformation.resetInvalidPinCount(this.sourceCustInfo.getCustomerId());
            }
    ...
    private boolean transferFundsP2P(long sourceAccountId, long destAccountId, double amount) throws SQLException {
        Connection cn = null;
        PreparedStatement stm = null;
        StringBuilder sb = null;
        int dbStatusCode = 0;
        ...
        try {
            ...
            stm = cn.prepareStatement(String.format(sqlQuery, amount, sourceAccountId));
            ...
            stm = cn.prepareStatement(String.format(sqlQuery, amount, destAccountId));
    ...
    private boolean transferFundsOwnMeToMeGoal(long sourceAccountId, int goalId, double amount, String referenceId, String sourceMsisdn)
            throws SQLException {
        Connection cn = null;
        PreparedStatement stm = null;
        StringBuilder sb = null;
        ...
        try {
            ...
            // Debit the Customer's Main Wallet and Credit the Customer's Me2Me Wallet
            sb = new StringBuilder();
            ...
            stm = cn.prepareStatement(String.format(sqlQuery, amount, amount, sourceAccountId));
            ...
            stm = cn.prepareStatement(String.format(sqlQuery, referenceId, sourceMsisdn, amount, goalId, amount, goalId, goalId));
    ...
    private boolean transferFundsOthersMeToMeGoal(long sourceAccountId, long destAccountId, int goalId, double amount, String referenceId,
    ...
    private boolean transferFundsRedeemMeToMeGoal(long sourceAccountId, int goalId, double amount) throws SQLException {
        Connection cn = null;
        PreparedStatement stm = null;
        StringBuilder sb = null;
        int dbStatusCode = 0;
        ...
        try {
            ...
            // Credit the Me2Me Goal
            sb = new StringBuilder();
            sb.append("UPDATE ME2ME_GOALS SET IS_REDEEMED = 1, DATE_REDEEMED = SYSDATE(), AMOUNT_REDEEMED=%f WHERE GOAL_ID = %d ");
            sqlQuery = sb.toString();
    ...
    private boolean transferFundsStopMeToMeGoal(long sourceAccountId, int goalId, double amount, String reasonStopped) throws SQLException {
        Connection cn = null;
        PreparedStatement stm = null;
        StringBuilder sb = null;
        int dbStatusCode = 0;
        ...
        try {
            ...
            stm = cn.prepareStatement(String.format(sqlQuery, reasonStopped, goalId));
    ...
    private boolean transferFundsRebate(long sourceAccountId, int goalId, double amount, String withdrawReason) throws SQLException {
        Connection cn = null;
        PreparedStatement stm = null;
        StringBuilder sb = null;
        int dbStatusCode = 0;
    ...
    private void processMBAL() {
        ...
        try {
            ...
            HelperUtils.writeToLogFile("InBoundMessages", String.format("%s~%s~%s", referenceId, sourceMsisdn, originalRequest));

            this.sourceCustInfo = CustomerInformation.getCustomerAccountInfo(sourceMsisdn);
            ...
            if (pinCode.equalsIgnoreCase(HelperUtils.getDefaultPinCode(SystemConfigInfo.getMinPasswordLen(),
            ...
            ValidationReturnInformation validSrcCustInfo = CustomerInformation.validateCustomerAccountInfo(sourceCustInfo, true);
            if (!validSrcCustInfo.isPassedValidation()) {
                HelperUtils.sendSMS(sourceMsisdn, validSrcCustInfo.getValidationMessage(), referenceId);
                return;
            }

            if (sourceCustInfo.getAccountInfo() == null || sourceCustInfo.getAccountInfo().getAccountId() == 0) {
            ...
            this.destMessage = String.format("REF: %s: Your available balance as at %s was %,.2f%s", referenceId, currDateStr,
                    sourceCustInfo.getAccountInfo().getBookBalance(), currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
        }
        catch (Exception ex) {
            HelperUtils.writeToLogFile("Server", "ERR: " + ex.getMessage() + " TRACE: " + ex.getStackTrace());
        ...
        try {
            ...
            if (!validatePinCode(pinCode)) {
                this.destMessage = "You have entered an Invalid PIN. Note that "
                        + Integer.toString(SystemConfigInfo.getInvalidPasswordLock())
            ...
            if (this.sourceCustInfo.getAccountInfo() == null || this.sourceCustInfo.getAccountInfo().getBookBalance() < amount) {
                this.destMessage = "You do not have sufficient funds in your account.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            destCustInfo = CustomerInformation.getCustomerAccountInfo(destMsisdn);
            ValidationReturnInformation retValidDestCust = CustomerInformation.validateCustomerAccountInfo(destCustInfo, false);
            if (!retValidDestCust.isPassedValidation()) {
                HelperUtils.sendSMS(sourceMsisdn, retValidDestCust.getValidationMessage(), referenceId);
                return;
            }

            if (destCustInfo.getAccountInfo() == null || destCustInfo.getAccountInfo().getAccountId() == 0) {
                this.destMessage = "The destination Account information could not be verified.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            boolean retTransfer = transferFundsP2P(sourceCustInfo.getAccountInfo().getAccountId(), destCustInfo.getAccountInfo()
                    .getAccountId(), amount);
            if (!retTransfer) {
            ...
            String currency = SystemConfigInfo.getCurrencyCode();
            double sourceBalAfter = sourceCustInfo.getAccountInfo().getBookBalance() - amount;
            double destBalAfter = destCustInfo.getAccountInfo().getBookBalance() + amount;

            this.destMessage = String.format("REF: %s: You have sent %,.2f%s to %s. Your available balance is: %,.2f%s.", referenceId,
                    amount, currency,
                    CustomerInformation.getDisplayNames(destCustInfo, false), sourceBalAfter, currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

            this.destMessage = String.format("REF: %s: You have received %,.2f%s from %s. Your available balance is: %,.2f%s.",
                    referenceId, amount, currency,
                    CustomerInformation.getDisplayNames(sourceCustInfo, false), destBalAfter, currency);
    ...
    private void processDPST() {
        ...
        try {
            ...
            this.destMessage = String.format("REF: %s: You have accepted deposit of %,.2f%s from %s. Your available balance is: %,.2f%s.",
                    referenceId, amount, currency,
                    CustomerInformation.getDisplayNames(destCustInfo, false), sourceBalAfter, currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

            this.destMessage = String.format(
                    "REF: %s: Please Give %,.2f%s cash to %s as your deposit. Your available balance is: %,.2f%s.", referenceId, amount,
                    currency,
                    CustomerInformation.getDisplayNames(sourceCustInfo, true), destBalAfter, currency);
            HelperUtils.sendSMS(destMsisdn, destMessage, referenceId);
    ...
    private void processPINC() {
        ...
        try {
            ...
            if (newPassword.length() < SystemConfigInfo.getMinPasswordLen() && SystemConfigInfo.getMinPasswordLen() != 0) {
                this.destMessage = "Your PIN is shorter than the recommended length.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            if (newPassword.length() > SystemConfigInfo.getMaxPasswordLen() && SystemConfigInfo.getMaxPasswordLen() != 0) {
                this.destMessage = "Your PIN is longer than the recommended length.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            String defaultPinCode = HelperUtils.getDefaultPinCode(SystemConfigInfo.getMinPasswordLen(),
                    SystemConfigInfo.getMaxPasswordLen());
            ...
            boolean retUpdate = CustomerInformation.changePinCode(this.sourceCustInfo.getCustomerId(), newPassword);
    ...
    private void processKYCR() {
        ...
        try {
            ...
            CustomerInformation existingCustInfo = CustomerInformation.getCustomerAccountInfo(phoneMsisdn);
            ...
            otherNames = transElements[3].trim().replace(asteriskChar, spaceChar);
            physicalAddress = transElements[4].trim().replace(asteriskChar, spaceChar);
            homeDistrict = transElements[5].trim().replace(asteriskChar, spaceChar);
            idType = transElements[6].trim().replace(asteriskChar, spaceChar);
            idNo = transElements[7].trim().replace(asteriskChar, spaceChar);
            nationality = transElements[8].trim().replace(asteriskChar, spaceChar);
            dateOfBirthStr = transElements[9].trim().replace(asteriskChar, spaceChar);
            dateOfBirth = new SimpleDateFormat("ddMMyyyy").parse(dateOfBirthStr);
            placeOfBirth = transElements[10].trim().replace(asteriskChar, spaceChar);

            String defaultPinCode = HelperUtils.getDefaultPinCode(SystemConfigInfo.getMinPasswordLen(),
                    SystemConfigInfo.getMaxPasswordLen());

            boolean retCreateCust = CustomerInformation.createCustomerAccount(phoneMsisdn, lastName, otherNames, physicalAddress,
                    homeDistrict, idType, idNo, nationality, dateOfBirth, placeOfBirth, defaultPinCode, sourceCustInfo.getCustomerId());
            ...
            CustomerInformation newCustInfo = CustomerInformation.getNewCustomerInfo(phoneMsisdn);
            if (newCustInfo == null) {
                this.destMessage = "The system encountered problems while Registering the new Customer.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            ...
            boolean retCreateAccount = CustomerInformation.createAccount(newCustInfo.getCustomerId(), newCustInfo.getAccountTypeBitmap());
            if (!retCreateAccount) {
                this.destMessage = "The system encountered problems while Registering the new Customer.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            ...
            String fullNames = lastName.concat(" ").concat(otherNames).toUpperCase();

            this.destMessage = "The New Customer: " + fullNames + " of Phone: " + phoneMsisdn + " has been Registered Successfully.";
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

            this.destMessage = "Welcome to AppLab Money. Your PIN is " + defaultPinCode
    ...
    private void processWDRW() {
        ...
        try {
            ...
            destCustInfo = CustomerInformation.getCustomerAccountInfoByAgentCode(agentCode);
            ValidationReturnInformation retValidDestCust = CustomerInformation.validateCustomerAccountInfo(destCustInfo, false);
            if (!retValidDestCust.isPassedValidation()) {
                HelperUtils.sendSMS(sourceMsisdn, retValidDestCust.getValidationMessage(), referenceId);
                return;
            ...
            this.destMessage = String.format("REF: %s: You have Withdrawn %,.2f%s at %s. Your available balance is: %,.2f%s.", referenceId,
                    amount, currency,
                    CustomerInformation.getDisplayNames(destCustInfo, true).toUpperCase(), sourceBalAfter, currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

            this.destMessage = String.format("REF: %s: Please give %,.2f%s to %s as withdrawal. Your available balance is: %,.2f%s.",
                    referenceId, amount, currency,
                    CustomerInformation.getDisplayNames(sourceCustInfo, false).toUpperCase(), destBalAfter, currency);
            HelperUtils.sendSMS(destMsisdn, destMessage, referenceId);
    ...
    private void processSTMT() {
        try {
            ...
            HelperUtils.writeToLogFile("InBoundMessages", String.format("%s~%s~%s", referenceId, sourceMsisdn, requestCommand));
        }
        catch (Exception ex) {
            HelperUtils.writeToLogFile("Server", "ERR: " + ex.getMessage() + " TRACE: " + ex.getStackTrace());
        }
    ...
    private void processCRGL() {
        ...
        try {
            ...
            SimpleDateFormat inputDateFormatter = new SimpleDateFormat("ddMMyyyy");
            SimpleDateFormat outputDateFormatter = new SimpleDateFormat("dd-MMM-yyyy");
            ...
            if (targetAmount < 50000.00) {
                this.destMessage = "The target amount is below the minimum thresh-hold. It has to be at least 50,000.00UGX. Please contact Customer Service for more Information.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            // Validate Maturity Date: Enforce 3 Months savings
            Calendar cal = Calendar.getInstance();
            cal.add(Calendar.MONTH, 3);

            if (maturityDate.before(cal.getTime())) {
                this.destMessage = "The Maturity Date or Period is Invalid. It has to be at least 3 months in the future. Please contact Customer Service for more Information.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            // Check whether the guy is activated
            boolean retActivatedMe2Me = MeToMeCommon.checkMeToMeActivation(sourceCustInfo.getCustomerId());
            ...
            MeToMeGoals activeGoals = MeToMeGoals.getActiveGoalsByCustomerAndGoalType(sourceCustInfo.getCustomerId(), goalType);
            if (activeGoals.size() > 0) {
                this.destMessage = String.format(
                        "You already have %d active goals of the same type. Multiple goals of the same type are currently not supported",
                        activeGoals.size());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            boolean retCreateGoal = MeToMeGoal.createMeToMeGoal(sourceCustInfo, goalType, targetAmount, maturityDate, alertOption,
                    liquidityOption);

            if (!retCreateGoal) {
                this.destMessage = "The System encountered a problem while creating your Me2Me goal target. Please contact Customer Service for more Information.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            this.destMessage = String.format(
                    "You have successfully created a %s goal target of %,.2f%s maturing on %s. You can now make savings to this goal.",
                    goalType.toUpperCase(), targetAmount, SystemConfigInfo.getCurrencyCode(), outputDateFormatter.format(maturityDate)
                            .toUpperCase());
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
        }
        catch (Exception ex) {
    ...
    private void processMTOM() {
        ...
        try {
            ...
            MeToMeGoals activeGoals = MeToMeGoals.getActiveGoalsByCustomerAndGoalType(destCustInfo.getCustomerId(), goalType);

            if (activeGoals.size() < 1) {
                if (isOwnPayment) {
                    this.destMessage = String.format("You don't have an Active Me2Me Goal for: %s. You must first create a goal.",
                            goalType.toUpperCase());
                    HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                }
                else {
                    this.destMessage = String.format(
                            "The Beneficiary %s does not have an Active %s Me2Me Goal. Please ask the beneficiary to first create a goal.",
                            CustomerInformation.getDisplayNames(destCustInfo, false).toUpperCase(), goalType.toUpperCase());
                    HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

                    // Notify the Beneficiary Customer
                    MeToMeGoals allActiveGoals = MeToMeGoals.getActiveGoalsByCustomerId(destCustInfo.getCustomerId());
                    if (allActiveGoals.size() < 1) {
                        this.destMessage = String
                                .format("The Sponsor %s attempted to Pay %,.2f%s into your %s Me2Me Goal, but you don't have any active goals. Please first create the goal then notify the sponsor.",
                                        CustomerInformation.getDisplayNames(sourceCustInfo, false).toUpperCase(), amount,
                                        SystemConfigInfo.getCurrencyCode(), goalType.toUpperCase());
                        HelperUtils.sendSMS(destMsisdn, destMessage, referenceId);
                    }
                    else {
                        // Get the Active Goals
                        String activeGoalsList = "";
                        for (int goalsIndex = 0; goalsIndex < allActiveGoals.size(); goalsIndex++) {
                            activeGoalsList = activeGoalsList.concat(allActiveGoals.get(goalsIndex).getGoalName());
                        ...
                        this.destMessage = String
                                .format("The Sponsor %s attempted to Pay %,.2f%s into your %s Me2Me Goal, but your Active goals are: %s. Please notify the sponsor.",
                                        CustomerInformation.getDisplayNames(sourceCustInfo, false).toUpperCase(), amount,
                                        SystemConfigInfo.getCurrencyCode(), goalType.toUpperCase(), activeGoalsList.toUpperCase());
                        HelperUtils.sendSMS(destMsisdn, destMessage, referenceId);
                    }
                }
                return;
            ...
            // Otherwise, Pick the First Goal Type: There will always be 1 goal per category for now
            ...
            if (isOwnPayment) {
                retTransfer = transferFundsOwnMeToMeGoal(sourceCustInfo.getAccountInfo().getAccountId(), targetGoal.getGoalId(), amount,
                        referenceId, sourceCustInfo.getMsisdn());
            }
            else {
                retTransfer = transferFundsOthersMeToMeGoal(sourceCustInfo.getAccountInfo().getAccountId(), destCustInfo.getAccountInfo()
                        .getAccountId(), targetGoal.getGoalId(), amount, referenceId, sourceCustInfo.getMsisdn());
            }
            ...
            double remainingTarget = targetGoal.getTargetAmount() - accruedGoalBalance;

            if (isOwnPayment) {

                this.destMessage = String.format(
                        "REF: %s: You have sent %,.2f%s to your %s Me2Me goal. The available balance in your main account is: %,.2f%s. ",
                        referenceId, amount, currency,
                        targetGoal.getGoalName().toUpperCase(), sourceBalAfter, currency);
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

                this.destMessage = String
                        .format("The Accrued Balance for your %s Goal is: %,.2f%s. You are left with %,.2f%s to reach your target amount of: %,.2f%s that will mature on %s.",
                                targetGoal.getGoalName(), accruedGoalBalance, currency,
                                remainingTarget, currency, targetGoal.getTargetAmount(), currency,
                                new SimpleDateFormat("dd-MMM-yyyy").format(targetGoal.getMaturityDate()).toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
            }
            else {

                this.destMessage = String.format(
                        "REF: %s: You have sent %,.2f%s to the %s Me2Me Goal for %s. Your available balance is: %,.2f%s. ", referenceId,
                        amount, currency,
                        targetGoal.getGoalName().toUpperCase(), CustomerInformation.getDisplayNames(destCustInfo, false).toUpperCase(),
                        sourceBalAfter, currency);
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

                this.destMessage = String
                        .format("REF: %s: The Sponsor %s has sent %,.2f%s to your %s Me2Me Goal. The Accrued Balance for the Goal is now: %,.2f%s. ",
                                referenceId, CustomerInformation.getDisplayNames(sourceCustInfo, false).toUpperCase(), amount, currency,
                                targetGoal.getGoalName().toUpperCase(), accruedGoalBalance, currency);
                HelperUtils.sendSMS(destMsisdn, destMessage, referenceId);
            }
        }
        catch (Exception ex) {
    ...
    private void processGBAL() {
        ...
        try {
            ...
            MeToMeGoals activeGoals = MeToMeGoals.getActiveGoalsByCustomerAndGoalType(sourceCustInfo.getCustomerId(), goalType);
            if (activeGoals.size() < 1) {
                this.destMessage = String.format("You don't have an Active Goal for: %s. You must first create a goal.",
                        goalType.toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            // Pick the First Goal Type: There will always be 1 goal per category for now
            targetGoal = activeGoals.get(0);
...
//            this.destMessage = String
//                    .format("The Accrued Balance for your %s Goal is: %,.2f%s. This is %.2f percent of your target amount of: %,.2f%s that will mature on %s.",
//                            targetGoal.getGoalName(), accruedGoalBalance, currency,
//                            ((accruedGoalBalance / targetGoal.getTargetAmount()) * 100), targetGoal.getTargetAmount(), currency,
//                            new SimpleDateFormat("dd-MMM-yyyy").format(targetGoal.getMaturityDate()).toUpperCase());
    ...
    private void processTENQ() {
        ...
        try {
            ...
            MeToMeGoalTransactions meToMeGoalTrans = targetGoal.getGoalTransactions();

            if (meToMeGoalTrans == null) {
                this.destMessage = String.format("You don't have any transactions for the %s goal.",
                        goalType.toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
    ...
    private void processREDM() {
        ...
        try {
            ...
            // Validate the GoalType or Name: get Unredeemed Goals for the Customer
            goalType = transElements[1].trim();
            MeToMeGoals unRedeemedGoals = MeToMeGoals.getUnRedeemedGoalsByCustomerAndGoalType(sourceCustInfo.getCustomerId(), goalType);
            if (unRedeemedGoals.size() < 1) {
                this.destMessage = String.format("You don't have an Active or Matured %s Goal. You must first create a goal.",
                        goalType.toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            if (!targetGoal.isMatured()) {
                if (targetGoal.getMaturityDate().after(currDate)) {
                    this.destMessage = String
                            .format("Your %s goal of target amount: %,.2f%s has not yet Matured. It will mature on %s. Please contact Customer Care for more information.",
                                    goalType.toUpperCase(), targetGoal.getTargetAmount(), currency, df.format(targetGoal.getMaturityDate()));
                    HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                }
                else {
                    this.destMessage = String
                            .format("Your %s goal of target amount: %,.2f%s has Matured and will be ready for Redemption within 24 hours. Try to redeem later. Please contact Customer Care for more information.",
                                    goalType.toUpperCase(), targetGoal.getTargetAmount(), currency, df.format(targetGoal.getMaturityDate()));
                    HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                }
                return;
            ...
            if (targetGoal.isRedeemed()) {
                this.destMessage = String
                        .format("The %s Goal you are trying to redeem is already redeemed. The redemption was on %s. Please contact Customer Care for assistance.",
                                targetGoal.getGoalName().toUpperCase(), df.format(targetGoal.getDateRedeemed()).toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            // Process Redemption
            boolean retRedeemGoal = transferFundsRedeemMeToMeGoal(sourceCustInfo.getAccountInfo().getAccountId(), targetGoal.getGoalId(),
                    accruedGoalBalance);
            ...
            // Get the final expected Book Balance. A second trip to the database would be more accurate
            double bookBalance = sourceCustInfo.getAccountInfo().getBookBalance() + accruedGoalBalance;
            double achievedPercentage = (accruedGoalBalance / targetGoal.getTargetAmount()) * 100;

            this.destMessage = String
                    .format("REF: %s: A LumpSum amount of %,.2f%s from your matured %s Goal was credited back into your main account. Your available balance in the main account is %,.2f%s.",
                            referenceId, accruedGoalBalance, currency, targetGoal.getGoalName().toUpperCase(), bookBalance, currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

            // I will have a function for doing the analysis on the savings and give the proper message
            this.destMessage = String
                    .format("Congratulations! You managed to incubate your Me2Me %s Goal of %,.2f%s until maturity and achieved %.2f percent of your target. You have earned a reward of %,d credit points.",
                            targetGoal.getGoalName().toUpperCase(), targetGoal.getTargetAmount(), currency, achievedPercentage,
                            Math.round(creditPoints));
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
        }
        catch (Exception ex) {
            HelperUtils.writeToLogFile("Server", "ERR: " + ex.getMessage() + " TRACE: " + ex.getStackTrace());
    ...
    private void processACTV() {
        ...
        try {
            ...
            if (confirmPinCode.length() < SystemConfigInfo.getMinPasswordLen() && SystemConfigInfo.getMinPasswordLen() != 0) {
                this.destMessage = "Your PIN is shorter than the recommended length.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            if (confirmPinCode.length() > SystemConfigInfo.getMaxPasswordLen() && SystemConfigInfo.getMaxPasswordLen() != 0) {
                this.destMessage = "Your PIN is longer than the recommended length.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            boolean retActivateMe2Me = MeToMeCommon.activateMe2Me(sourceCustInfo);
            ...
            this.destMessage = "You have successfully activated your Me2Me wallet. You can now create goal targets and make deferred payments to yourself.";
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
        }
        catch (Exception ex) {
            HelperUtils.writeToLogFile("Server", "ERR: " + ex.getMessage() + " TRACE: " + ex.getStackTrace());
    ...
    private void processSTOP() {
        ...
        try {
            ...
            MeToMeGoals activeGoals = MeToMeGoals.getActiveGoalsByCustomerAndGoalType(sourceCustInfo.getCustomerId(), goalType);
            if (activeGoals.size() < 1) {
                this.destMessage = String.format("You don't have an Active %s Goal. You must first create a goal.", goalType.toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            reasonStopped = transElements[2].trim().replace(asteriskChar, spaceChar);
            ...
            // Process Stoppage
            boolean retStopGoal = transferFundsStopMeToMeGoal(sourceCustInfo.getAccountInfo().getAccountId(), targetGoal.getGoalId(),
            ...
            this.destMessage = String
                    .format("REF: %s: The accumulated amount of %,.2f%s from your Stopped %s Goal was credited back into your main account. Your available balance in the main account is %,.2f%s.",
                            referenceId, accruedGoalBalance, currency, targetGoal.getGoalName().toUpperCase(), bookBalance, currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

            // I will have a function for doing the analysis on the savings and give the proper message
            this.destMessage = String
                    .format("Unfortunately, you did not Incubate your Me2Me %s Goal of %,.2f%s until maturity. You stopped at %.2f percent of your target. Please give us feedback of your experience to help us better the Me2Me Product Features.",
                            targetGoal.getGoalName().toUpperCase(), targetGoal.getTargetAmount(), currency, achievedPercentage);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
        }
        catch (Exception ex) {
    ...
    private void processREBT() {
        ...
        try {
            ...
            if (targetGoal.getLiquidityOption().equalsIgnoreCase(HelperUtils.LIQUIDITY_OPTION_NAME_NONE)) {
                this.destMessage = String.format(
                        "Your %s Goal does not allow for Liquidity. Please be patient and wait for the goal to mature on %s.", targetGoal
                                .getGoalName().toUpperCase(), new SimpleDateFormat("dd-MMM-yyyy").format(targetGoal.getMaturityDate())
                                .toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            withdrawReason = transElements[2].trim().replace(asteriskChar, spaceChar);
            ...
            String currency = SystemConfigInfo.getCurrencyCode();
            double accruedGoalBalance = targetGoal.getAccruedAmount();
            double percentageSaved = (accruedGoalBalance / targetGoal.getTargetAmount()) * 100;

            if (percentageSaved < HelperUtils.REBATE_MIN_SAVED_PERCENT) {
                this.destMessage = String
                        .format("You are required to have achieved at least %.2f percent of the target amount of %,.2f of your %s goal in order to qualify for liquidity.",
                                HelperUtils.REBATE_MIN_SAVED_PERCENT, targetGoal.getTargetAmount(), currency, targetGoal.getGoalName()
                                        .toUpperCase());
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            double maxAllowedLiquidity = (HelperUtils.REBATE_MAX_ALLOWED_PERCENT_OF_BALANCE / 100) * accruedGoalBalance;

            if ((percentageOfBalance > HelperUtils.REBATE_MAX_ALLOWED_PERCENT_OF_BALANCE)
                    && (!targetGoal.getLiquidityOption().equalsIgnoreCase(HelperUtils.LIQUIDITY_OPTION_NAME_FULL))) {
                this.destMessage = String
                        .format("Your %s goal allows for %s Liquidity of upto %.2f percent of your accumulated balance. You currently qualify for a maximum liquidity of %,.2f%s.",
                                targetGoal.getGoalName().toUpperCase(), targetGoal.getLiquidityOption().toUpperCase(),
                                HelperUtils.REBATE_MAX_ALLOWED_PERCENT_OF_BALANCE, maxAllowedLiquidity, currency);
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            // Verify Amounts: Still consider the total accrued Balance during Reconciliation
            if ((accruedGoalBalance < amount) || (sourceCustInfo.getAccountInfo().getBlockedBalance() < accruedGoalBalance)) {
            ...
            // Process Rebate
            boolean retRebate = transferFundsRebate(sourceCustInfo.getAccountInfo().getAccountId(), targetGoal.getGoalId(), amount,
                    withdrawReason);
            ...
            // Get the final expected Book Balance. A second trip to the database would be more accurate
            double bookBalance = sourceCustInfo.getAccountInfo().getBookBalance() + amount;
            double newAccruedGoalBalance = accruedGoalBalance - amount;

            this.destMessage = String
                    .format("REF: %s: A rebate amount of %,.2f%s from your %s Goal was credited back into your main account. Your available balance in the main account is %,.2f%s.",
                            referenceId, amount, currency, targetGoal.getGoalName().toUpperCase(), bookBalance, currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

            this.destMessage = String
                    .format("A rebate amount of %,.2f%s has been debited from your %s Goal as per your liquidity request. The Accrued Balance for the goal has decreased to %,.2f%s.",
                            amount, currency, targetGoal.getGoalName().toUpperCase(), newAccruedGoalBalance, currency);
            HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
        }
        catch (Exception ex) {
            HelperUtils.writeToLogFile("Server", "ERR: " + ex.getMessage() + " TRACE: " + ex.getStackTrace());
    ...
    private void processINVT() {
        ...
        try {
            ...
            if (pinCode.length() < SystemConfigInfo.getMinPasswordLen() && SystemConfigInfo.getMinPasswordLen() != 0) {
                this.destMessage = "Your PIN is shorter than the recommended length.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }

            if (pinCode.length() > SystemConfigInfo.getMaxPasswordLen() && SystemConfigInfo.getMaxPasswordLen() != 0) {
                this.destMessage = "Your PIN is longer than the recommended length.";
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            for (int i = 0; i < phoneNumbers.length; i++) {
                ...
                friendCustInfo = CustomerInformation.getCustomerAccountInfo(friendMsisdn);
                ...
                if (friendCustInfo == null) {
                    this.destMessage = String
                            .format("Please support me MTN Mobile Money and join my Zimba network so that we can borrow and lend each other cash using the mobile phone. Thanks. %s.",
                                    CustomerInformation.getDisplayNames(sourceCustInfo, false));
                    HelperUtils.sendSMS(friendMsisdn, destMessage, referenceId);
                }
                else {
                    this.destMessage = String
                            .format(
                                    "Please join my Zimba network so that we can borrow and lend each other cash using Mobile Money. Thanks. %s.",
                                    CustomerInformation.getDisplayNames(sourceCustInfo, false));
                    HelperUtils.sendSMS(friendMsisdn, destMessage, referenceId);
                }
                ...
                if (friendCustInfo == null) {
                    this.destMessage = String.format("Your Invitation to %s to join your Zimba network has been sent successfully.",
                            phoneNumbers[i]);
                }
                else {
                    this.destMessage = String.format("Your Invitation to %s to join your Zimba network has been sent successfully.",
                            CustomerInformation.getDisplayNames(friendCustInfo, false));
                }
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);   
            }
        }
    ...
    private void processNTACTV() {
        ...
        try {
            ...
            boolean retValCheckZimbaActivation = ZimbaCommon.checkZimbaActivation(sourceCustInfo.getCustomerId());
            ...
            boolean retActivateZimba = ZimbaCommon.activateZimba(sourceCustInfo, networkSize, networkRole);
        ...
        try {
            ...
            for (int i = 0; i < phoneNumbers.length; i++) {
                ...
                if (friendCustInfo != null) {
                    ...
                    ZimbaNetwork existingNetwork = ZimbaNetworks.getNetworkForCustomerToFriend(sourceCustInfo.getCustomerId(),
                            friendCustInfo.getCustomerId());
                    
                    if (existingNetwork != null) {
                        
                        this.destMessage = String
                                .format("You are already connected to: %s since %s.",
                                        CustomerInformation.getDisplayNames(friendCustInfo, false),
                                        new SimpleDateFormat("dd-MMM-yyyy").format(existingNetwork.getDateConnected()));
                        HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                        return;
                    }
                }

                // Also check whether there is a pending invitation to the friend
                boolean isReminderInvitation = false;
                ZimbaInvitations pendingInvitations = ZimbaInvitations.getPendingInvitationsFromCustomerIdToMsisdn(
                ...
                if (!isReminderInvitation) {
                    ...
                    zimbaInvitation.setCustomerId(sourceCustInfo.getCustomerId());
                    zimbaInvitation.setFriendMsisdn(friendMsisdn);

                    boolean retVal = ZimbaInvitations.createZimbaInvitation(zimbaInvitation);
                    if (retVal) {
                        // Do Notify: But I have already told Customer that invite was sent???? -:)
                    }
                }
            }
        }
    ...
    private void processNTRESPINV() {
        ...
        try {
            ...
            CustomerInformation invitorCustInfo = CustomerInformation.getCustomerAccountInfo(invitorMsisdn);
            ...
            // Check whether they are already connected
            ZimbaNetwork existingNetwork = ZimbaNetworks.getNetworkForCustomerToFriend(invitorCustInfo.getCustomerId(),
                    sourceCustInfo.getCustomerId());
            if (existingNetwork != null) {
                this.destMessage = String
                        .format("You are already connected to: %s since %s.",
                                CustomerInformation.getDisplayNames(invitorCustInfo, false),
                                new SimpleDateFormat("dd-MMM-yyyy").format(existingNetwork.getDateConnected()));
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);
                return;
            }
            ...
            ZimbaInvitations zimbaInvitations = ZimbaInvitations.getPendingInvitationsFromCustomerIdToMsisdn(
                    invitorCustInfo.getCustomerId(), sourceCustInfo.getMsisdn());
            ...
            if (response.toUpperCase().startsWith("ACCEPT")) {
                respInvite = ZimbaInvitations.respondToZimbaInvitation(zimbaInvitation, 1, sourceCustInfo.getCustomerId(), friendshipLevel);
            }
            else
            {
                respInvite = ZimbaInvitations.respondToZimbaInvitation(zimbaInvitation, 2, sourceCustInfo.getCustomerId(), friendshipLevel);
            }
            ...
            if (response.toUpperCase().startsWith("ACCEPT")) {
                ...
                this.destMessage = String
                        .format("Congratulations! You are now connected to: %s. Your %s Zimba Group is growing!.",
                                CustomerInformation.getDisplayNames(invitorCustInfo, false), friendshipLevel);
                HelperUtils.sendSMS(sourceMsisdn, destMessage, referenceId);

                // Message to the Invitor:
                // TODO: I need to be able to set friendship levels for the Invitor also
                this.destMessage = String
